...
export interface FieldComponentProps {
	...
	/**
	 * ISO country codes to offer. The app's version reads these from the
	 * backend service DTO (`useServiceCountries`); the package stays
	 * backend-free, so hosts pass the resolved list (or omit for all).
	 */
	restrictTo?: string[]
...
export const Component: React.FC<FieldComponentProps> = ({
	...
	const { options } = useCountry(undefined, restrictTo)
	const [modalVisible, setModalVisible] = useState(false)

	const {
		field: { onChange, value },
		fieldState: { error, isTouched },
	} = useController({ name, control, rules, defaultValue: defaultCountryCode })

	const hasError = Boolean(error?.message) && isTouched
	const selected = options.find((o) => o.value?.toUpperCase() === (value ?? defaultCountryCode)?.toUpperCase())
